...
int main(int argc, char* argv[])
{
	...
	while(bzero(cmd, 128),(read_len = read(0, cmd, 128)) > 0 )
	{
		...
		trim_space(cmd);
		send_len = strlen(cmd);
		//send the len of the cmd , then send the string to server;
		send(fd_client, &send_len, sizeof(int), 0);
		sendn(fd_client, cmd, send_len);
		if(strncmp("cd", cmd, 2) == 0)
		{
			//compare first if string the cd , then server will send something back which will contain cd ;
			system("clear");
			//recv the len ; and the recv the cd command fromt the server and then print it to the screebn;
		...
		}else if(strncmp("ls", cmd, 3) == 0)
		{
			...
			while(1)
			{
				//print ls one line by one line; use while to control the 
				//first recv the len of the buffer and then recv the 
				recv(fd_client, &recv_len, sizeof(int), 0);
				if(recv_len == 0)
				{
					break ;
